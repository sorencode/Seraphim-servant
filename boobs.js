...
const { fetchBoobs, fetchWaifuIm, fetchNekoBot } = require('../utils/api');
const { logInfo } = require('../utils/logger');

module.exports = {
    data: new SlashCommandBuilder()
        .setName('boobs')
        .setDescription('Delivers a random NSFW boobs Image/GIF')
    ...
    async execute(interaction, isButton = false, savedStyle = null) {
        ...
        const animeSources = [ { id: 'waifuim', tag: 'oppai' }, { id: 'nekobot', type: 'hboobs' } ];
        const realSources = [ { id: 'oboobs' }, { id: 'nekobot', type: 'boobs' } ];
        ...
        if (sourceObj.id === 'purrbot') {
            logInfo(`[/boobs] Selected API Source: Purrbot (${sourceObj.endpoint})`);
            ...
            logInfo(`[/boobs] Selected API Source: Oboobs`);
            imageData = await fetchBoobs(null);
        } else if (sourceObj.id === 'obutts') {
            logInfo(`[/boobs] Selected API Source: Obutts`);
            imageData = await fetchAss(null);
        } else if (sourceObj.id === 'nekobot') {
            logInfo(`[/boobs] Selected API Source: NekoBot (type: ${sourceObj.type})`);
        ...
        const customIdBase = style ? `refresh_boobs_${style}` : `refresh_boobs`;
