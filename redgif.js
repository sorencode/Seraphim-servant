...
module.exports = {
    ...
    async execute(interaction, isButton = false, tagOverride = null) {
        ...
        const metaEmbed = new EmbedBuilder()
            ...
            .setFooter({
                text: `${interaction.user.username} | RedGif • Anti-repeat`,
                iconURL: interaction.user.displayAvatarURL()
            });

        // ── Strategy 1: upload MP4 file directly (HD → SD fallback) ─────────
        ...
        // ── Strategy 2: send RedGif page URL as content so Discord shows the
        //    video preview via Open Graph (works even for large videos) ───────
        logWarn(`[/redgif] Upload failed for "${gifData.id}" — using OGP page URL`);
        metaEmbed
